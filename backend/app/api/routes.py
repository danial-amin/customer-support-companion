...
class RAGQueryResponse(BaseModel):
    ...
    answer: str
    context_sources: int
    sources: Optional[List[Dict[str, Any]]] = None  # Full source information
    error: Optional[str] = None
        ...
        if request.agent_type and request.agent_type != "auto":
            ...
            if request.agent_type == "rag":
                ...
                if result.get("answer"):
                    ...
                    if row_count == 0:
                        answer = "Query executed successfully, but no rows were found."
                    else:
                        answer = f"Query executed successfully. Found {row_count} row(s). See results table below."
                else:
            ...
            # If RAG was used, include sources in metadata
            if result.get("rag_result") and result.get("rag_result").get("sources"):
                metadata["rag_sources"] = result.get("rag_result").get("sources")
...
async def upload_file_get():
    ...
    return JSONResponse(
        ...
        content={
            ...
            "method": "Use POST with multipart/form-data",
            "example": "curl -X POST -F 'file=@document.pdf' http://localhost:8000/api/v1/rag/upload",
            "supported_formats": [".txt", ".pdf", ".doc", ".docx", ".md", ".csv"],
            "note": "If you're using the frontend, make sure to use the file upload form, not a direct browser GET request."
        }
    )


@router.options("/rag/upload")
async def upload_file_options():
    """Handle CORS preflight for file upload endpoint."""
    return JSONResponse(
        content={},
        headers={
...
async def upload_file(
    ...
    Note: This endpoint only accepts POST requests. GET requests will return "Method Not Allowed".
    """
    try:
        # Validate file type
